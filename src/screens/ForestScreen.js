...
export default function ForestScreen() {
    ...
    const [data, setData] = useState([]); // Dữ liệu API
    const [loading, setLoading] = useState(true); // Trạng thái loading
    const [selectedItem, setSelectedItem] = useState(null); // Mục đã chọn
    const [modalVisible, setModalVisible] = useState(false); // Trạng thái modal

    useEffect(() => {
        // Gọi API để lấy dữ liệu
        fetch("https://673d3fcb0118dbfe8606a071.mockapi.io/forest")
    ...
    // Xử lý khi nhấn vào nút "Bắt đầu khám phá"
        ...
        <View style={styles.card}>
            <ImageBackground source={{ uri: item.imageUrl }} style={styles.cardImage} resizeMode="cover">
                <View style={styles.cardOverlay}>
                    ...
                    <TouchableOpacity style={styles.button} onPress={() => handleExplore(item)}>
